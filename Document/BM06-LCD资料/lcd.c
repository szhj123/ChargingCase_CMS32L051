...
WriteComm(0x2a); // Set col address
WriteData(0); // Start col address
WriteData(0);
WriteData(0);
WriteData(79); // End col address
...
void DispFrame(void)
{
	...
	for(j=0;j<ROW-2;j++)
	{
		SendDataSPI(0xFF);SendDataSPI(0xFF);
	...
	for(n=0;n<ROW/PIC_HEIGHT;n++)         //n-row repeat count
	{
		for(i=0;i<PIC_HEIGHT;i++)
		{
			...
			for(k=0;k<COL/PIC_WIDTH;k++) //k-col repeat count
		    {
		    	...
		    	{
				    SendDataSPI((*(p+i*PIC_HEIGHT+j))>>8); 
...
void DispScaleHor2(void)
{
	...
	for(k=0;k<ROW/6;k++)
	{
		for(i=0;i<COL%32;i++){SendDataSPI(0x00);SendDataSPI(0x00);}
		for(i=0;i<32;i++)
		{
			for(j=0;j<COL/32;j++)	 
			{
	...
	for(k=0;k<ROW/6;k++)
	{
		for(i=0;i<COL%64;i++){SendDataSPI(0x00);SendDataSPI(0x00);}
		for(i=0;i<64;i++)
		{
			for(j=0;j<COL/64;j++)
			{
